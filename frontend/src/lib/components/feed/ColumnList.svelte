...
<div class="max-w-sm rounded-lg overflow-hidden shadow-lg m-2 bg-slate-100">
    ...
    <div class="px-6 py-4">
      <div class="h-auto font-bold text-s mb-2">
        <p class="text-ellipsis">
            <a href={Item.link} class="link link-hover text-ellipsis" target="_blank" on:click={()=> userAction(userId, Item.title, "viewed", "post", Item.link, Item.id, Item.keywords)}>
                    {@html Item.title}
            </a>
        </p>
    </div>
    ...
    <div class="flex flex-1 flex-row space-x-2 m-2">
        ...
        (userId, Item.title, "likes", "post", Item.link, Item.id, Item.keywords)}>
                ...
                <button class="btn btn-xs bg-transparent border border-black" type="button" on:click={() => userAction
        (userId, Item.title, "dislikes", "post", Item.link, Item.id, Item.keywords)}>
                ...
                <button class="btn btn-xs bg-transparent border border-black" type="button" on:click={() => userAction
        (userId, Item.title, "bookmark", "post", Item.link, Item.id, Item.keywords)}>
