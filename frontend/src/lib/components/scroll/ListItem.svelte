...
<div class="flex flex-col gap-x-2 rounded-md bg-slate-300 m-2">
    <div class="flex flex-col flex-wrap justify-between p-4 leading-normal">
        <h5 class="mb-2 text-1xl font-bold tracking-tight text-black">
            {Item.title}
        </h5>
        <div class="flex-1 min-w-0">
            <p
                class="text-sm font-medium text-black truncate hover:text-gray-400"
            >
                By {Item.author}, published on {Item.pubDate},
                {Item.publisher}
            </p>
        </div>
    ...
    {#if Item.category}
        <div class="flex flex-1 flex-row space-x-1 m-2">
